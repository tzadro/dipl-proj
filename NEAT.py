...
class Individual:  # Genome
	...
	def visualize(self):
		edges = [(connection.from_key, connection.to_key, round(connection.weight, 2)) for connection in
	...
	def new_connection(self, generation_innovations):
		...
		if num_connections == max_num_edges(num_nodes) - (max_num_edges(num_inputs) + max_num_edges(num_outputs)):
			return
		...
		while True:
			...
			if node2.key in config.input_keys or check_if_path_exists(node2.key, node1.key, self.connections, {}) or (
...
class Phenotype:  # Neural network
	def __init__(self, connections):
		...
		for connection in connections.values():
			...
			if not check_if_path_exists2(connection.to_key, connection.from_key, self.neurons, {}):
